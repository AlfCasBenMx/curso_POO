...
def build_clientes(*, n: int) -> list[dict[str, str]]:
    # Mantiene el formato de clientes.csv: id,nombre
    base = [
        "ACME SA",
...
def build_ventas(*, rng: random.Random, n: int, start: date, end: date, n_clientes: int) -> list[Venta]:
    ...
    for _ in range(n):
        ...
        ventas.append(Venta(id_cliente=id_cliente, fecha=fecha, importe=f"{imp}", categoria=categoria))
...
def build_ventas_sucio(*, base: list[Venta], take: int) -> list[dict[str, str]]:
...
def main() -> None:
    ...
    ventas_sucio = build_ventas_sucio(base=ventas, take=30)
